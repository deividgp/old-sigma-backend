...
export default async function addData() {
    const user1 = await User.create({ username: "Deividgp", password: "hi1234", email: "hi1234@gmail.com" });
    const server = await Server.create({ name: "Server1", description: "This is server number 1", OwnerId: user1.id });
    ...
    const user2 = await User.create({ username: "David", password: "hi1234", email: "hi1234@gmail.com" });

    const user3 = await User.create({ username: "Oriol", password: "hi1234", email: "hi1234@gmail.com" });
    ...
    const channel = await Channel.create({ ServerId: server.id, name: "Channel2" });
    ...
    const server2 = await Server.create({ name: "Server2", description: "This is server number 2", OwnerId: user1.id });
    await UserServers.create({ UserId: user1.id, ServerId: server2.id, joined: Date.now() });
